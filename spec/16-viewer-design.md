...
Focus horizon (#99): with a focus set, the node-link view may be bounded to a hop radius around it. The radius is session state, never a §16.4 address key: the address names what is being looked at, not how far around it the reader is looking. Hops are counted over the edges in view, so hiding a family narrows the horizon with it. What lies past the bound is not drawn and is not counted; the status line says the field continues, without a number (a running total of what is ahead is a progress reading, §3, §4). An edge with one end past the bound is drawn from its own plate outward and stops, carrying edge family alone: no arrowhead at an absent endpoint, no weight tick at a midpoint off screen (§16.2 A3, A5). A bound is never rendered as a node with no further relations. The bound is the node-link view's only: the list is A11's fallback and holds the whole field (a cut relation is a stub in the picture and would be nothing at all in a table), and the radius control stands down while the reader has chosen the list. The control stays live under §25.8's automatic fallback, where narrowing the radius is the one move that brings the field back under the threshold, and stays live even once the reader asks for the list the fallback was already showing: a fallback that took away the way out of itself would be a trap.

Selection emphasis (#99): while a selection stands on a node the field draws, the relations that do not touch it recede, so the ones that do read as the reader's own constellation. This is A9's focus feedback and carries no claim about the graph: it takes the one transition A9 has, settles rather than snaps, and collapses to zero under reduced motion. Presence is taken from the complement and never added to the selection's relations: stroke colour, dash and width are identical on both sides (they carry edge family alone, A3), and no edge gains a second mark (A5). Each family recedes *to* the sheet's own hairline rather than *by* one shared amount, because the families do not start level. The hairline is also the floor: no family falls below the contrast a rule already carries against the ground, and each family's amount is measured where a relation actually paints, on the field's own surface, not on the plates above it. A receded relation is still drawn, still answers a hover, and still holds its hit band; omission belongs to A11's fixed order and to the focus horizon, never here. The trail never recedes: a route touching the selection beside a trail that does not would render the route brighter than the trail, A7's inversion. Position and size do not change (A10), and the same drawn set settles into the same picture whether or not anything is selected (§27.8). The emphasis keys off the selection, so pointer and keyboard draw the identical picture, and the panel names every relation in words (A8).

Drawn presence (#99): that floor is the drawn relation's, not the recession's. A stroke too thin to paint reads exactly like one dimmed past the hairline, and the hit band is already held at screen scale, so a field fitted by zoom would leave the hand reaching for relations the eye has lost. Drawn smaller than itself, by the camera or by a frame narrower than the one the field was authored for (§16.4), a view holds its dash period at its own size and lifts every family's width together until the thinnest clears the floor. The lift is read from the view alone and is the same for every relation on the sheet, so it carries nothing about the graph (A10); the families keep their order and their ratios, because a floor that merged two of them would spend the width A3 gave them. A direction mark is sized in stroke widths, so the lift is divided back out of it and an arrowhead never outgrows the plate it points at; an end cap likewise, squared off while the lift is working rather than running half a lifted width back over the glyph its endpoint was trimmed to clear. Below the floor a relation has not thinned, it has gone, and going belongs to A11's fixed order and to the focus horizon. Under a pixel a stroke paints pale, not thin (the width is the coverage), so the selection's recession is divided by what the family actually lays down before it is applied. At full size that division is the identity; where even the whole amount cannot be spent, the quiet stands down and the selection still answers through the ring and the panel's words (A8).
