...
Drawn presence (#99): that floor is the drawn relation's, not the recession's. A stroke too thin to paint reads exactly like one dimmed past the hairline, and the hit band is already held at screen scale, so a field fitted by zoom would leave the hand reaching for relations the eye has lost. A view zoomed out lifts every family's width together until the thinnest clears the floor: the lift is read from the view alone and is the same for every relation on the sheet, so it carries nothing about the graph (A10), and the families keep their order and their ratios, because a floor that merged two of them would spend the width A3 gave them to save the colour. A direction mark is sized in stroke widths, so the lift is divided back out of it and an arrowhead never outgrows the plate it points at. Below that floor a relation has not thinned, it has gone — and going belongs to A11's fixed order and to the focus horizon.
