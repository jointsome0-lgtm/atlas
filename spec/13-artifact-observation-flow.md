...
A direct observation run is a writer (§25.6) and receipts each unit it processes into `state/receipts.jsonl` (§33.2) under `observe/<date-serial>#<n>`, date-serial like every receipt key (§34.6 pattern, never content-derived), with the same opened/processed discipline as an intake batch: an interrupted run resumes at its first unreceipted unit. A re-run over unchanged artifacts appends nothing: recognition reads the journals' own artifact rows (purgeable memory), never the receipts. The unit of processing per input kind (a commit, a file version, a declaration) is fixed at the observer's design pass, deliberately open here.
