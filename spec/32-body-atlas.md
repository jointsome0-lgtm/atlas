...
**No decision means no knowledge (§14.6).** With no confirmed decision the gated zone dimensions fold to `unknown`, capacities and `condition` alike. `fine` is a positive medical claim, exactly as gated as `chronic`: it enters only through a confirmed decision citing measurement evidence, so an unexamined zone never renders as a healthy one (#38).
...
The class also sets the deletion default (a classed record is purged when deleted, §34.1) and the id convention: classed records take non-descriptive date-serial ids at creation (§34.6).
