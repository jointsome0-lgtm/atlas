...
§32 remains an extension design and second-domain test, but Body Atlas implementation is frozen under #45. Phases 1–3 and their tickets must not depend on §32-specific code. During the freeze, excluded work is Body Atlas code, real health journals, silhouette implementation, medical-derived state, new body-specific features, fields, or modes, and public sample health data. Accordingly, §20's requirements to read pattern and zone frontmatter, fold body state, and embed silhouette projections are deferred; the active builder implements the knowledge-domain branches of those steps, while §20 remains the complete post-unfreeze contract. A §32 defect may still be corrected when it affects the generic core.
