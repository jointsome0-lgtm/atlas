...
def build(root, entries):
    for entry in entries:
        ...
        if kind == "dir":
            ...
            raise SystemExit("unknown setup kind " + kind)


def paths_only(messages):
    # §24.4 makes the location the stable half of a diagnostic; the English
    # after it is not a contract, and the port words it differently on purpose
    # rather than reproducing CPython repr punctuation.
    return [message.split(": ", 1)[0] for message in messages]


def run_op(instance, op):
    name = op["op"]
    if name == "preserve_bytes":
        return instance.preserve_bytes(op["path"], base64.b64decode(op["data"]))
    if name == "append_record":
        r = instance.append_record(op["path"], op["record"])
        return [r.relative_path, r.bytes_written, r.created]
    if name == "append_receipt":
        r = instance.append_receipt(op["key"], op["marker"], op["date"])
        return [r.relative_path, r.bytes_written, r.created]
    if name == "receipt_status":
        s = instance.receipt_status()
        return [sorted(s.opened), sorted(s.processed), sorted(s.interrupted)]
    if name == "read_json":
        return instance.read_json(
            op["path"], max_bytes=op["max_bytes"],
            delivered=op.get("delivered", False))
    if name == "read_delivered_json":
        d = instance.read_delivered_json(
            op["path"], max_bytes=op["max_bytes"],
            delivered=op.get("delivered", True))
        return [d.value, base64.b64encode(d.data).decode("ascii")]
    if name == "enforce_ceiling":
        atlas_io.enforce_ceiling(
            op["actual"], maximum=op["maximum"], kind=op["kind"],
            relative_path=op.get("path", "."))
        return None
    if name == "make_receipt_key":
        return atlas_io.make_receipt_key(op["source"], op["batch"], op["index"])
    if name == "path":
        p = instance.path(op["path"], allow_missing=op.get("allow_missing", False))
        return str(p.relative_to(instance.root))
    if name == "schema_errors":
        return paths_only(instance.schema_errors(
            op["value"], op["schema"], definition=op.get("definition")))
    if name == "validate_schema":
        instance.validate_schema(
            op["value"], op["schema"], definition=op.get("definition"))
        return None
    if name == "validate_format":
        return instance.validate_format(op["value"], definition=op.get("definition"))
    if name == "validate_raw":
        import validate_atlas
        # "text" hands each side the same bytes to parse with its own reader,
        # which is the only way a difference in key order can survive: a value
        # sent as JSON has already been through the sender's ordering.
        value = json.loads(op["text"]) if "text" in op else op["value"]
        try:
            return paths_only(
                validate_atlas.SchemaValidator(op["schema"]).validate(value))
        except validate_atlas.SchemaSubsetError as exc:
            return ["subset-error"]
    raise SystemExit("unknown op " + name)


def run_ops(instance, ops, observed):
    for op in ops:
        try:
            observed.append({"ok": run_op(instance, op)})
...
def snapshot(root):
    ...
    for path in sorted(root.rglob("*")):
        ...
        if path.is_symlink():
            ...
            found.append([rel, base64.b64encode(path.read_bytes()).decode("ascii")])
    return found
...
try:
    ...
    json.dump({"construct": str(exc), "ops": [], "tree": snapshot(root),
               "lock_left": False}, sys.stdout)
    raise SystemExit(0)

run_ops(instance, scenario.get("before", []), observed)
locked = scenario.get("locked", [])
if locked:
    try:
        with instance.lock():
            run_ops(instance, locked, observed)
    except atlas_io.AtlasIOError as exc:
        observed.append({"error": str(exc)})
run_ops(instance, scenario.get("after", []), observed)

# The lock file carries this process's own pid, so it can never match across
# implementations; whether one was left behind is what gets compared.
tree = [entry for entry in snapshot(root) if entry[0] != ".atlas-lock"]
json.dump({"construct": None, "ops": observed, "tree": tree,
           "lock_left": (root / ".atlas-lock").exists()}, sys.stdout)
`;
